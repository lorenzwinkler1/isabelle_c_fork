...
evaluation side (with the option \isa{quick{\isacharminus}and{\isacharminus}dirty} activated). In a nutshell, our new
implemented \isa{\isacommand{Haskell{\isacharunderscore}file}} command accepts other various
options:~\footnote{Note that \isa{\isacommand{old{\isacharunderscore}datatype}} is a command,
  whereas \isa{\isakeyword{datatype{\isacharunderscore}old}} is an option provided to the command
...
obtained from Haskabelle: operationally, \isa{\isacommand{Haskell{\isacharunderscore}file}} makes an
...
of the \isa{\isacommand{datatype}} package is not improving when being generated and
