...
\end{wrapfigure}
The \<^theory_text>\<open>record\<close> package creates an \<^theory_text>\<open>'\<sigma>\<close> extensible record type 
\<^theory_text>\<open>'\<sigma> control_state_ext\<close> where the \<^theory_text>\<open>'\<sigma>\<close> stands for extensions that were subsequently
``stuffed'' in them. On this basis, assignments, conditionals and loops are reformulated into 
\<^term>\<open>break\<close>-aware and \<^term>\<open>return\<close>-aware versions as shown in the figure aside.\<close>
...
extensible records --- in this case, just a global variable \<^term>\<open>k\<close> and a local variable 
...
generated definitions available subsequently in the logical context.

The definitions of the push-pop operations have also been generated by Clean for local variable
blocks in procedures together with the
 generation of the fancy syntax for assignments on global and local variables. For example,
\<open>i := 2\<close> is a syntax for the update code for local variables:
