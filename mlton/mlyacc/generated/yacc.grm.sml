...
structure MlyValue = 
struct
datatype svalue = VOID | ntVOID of unit ->  unit | UNKNOWN of unit ->  (string) | TYVAR of unit ->  (string) | PROG of unit ->  (string) | PREC of unit ->  (Header.prec) | INT of unit ->  (string) | IDDOT of unit ->  (string) | ID of unit ->  (string*Header.pos) | HEADER of unit ->  (string) | TY of unit ->  (string) | CHANGE_DEC of unit ->  ( ( Hdr.symbol list * Hdr.symbol list ) ) | CHANGE_DECL of unit ->  ( ( Hdr.symbol list * Hdr.symbol list )  list) | SUBST_DEC of unit ->  ( ( Hdr.symbol list * Hdr.symbol list ) ) | SUBST_DECL of unit ->  ( ( Hdr.symbol list * Hdr.symbol list )  list) | G_RULE_PREC of unit ->  (Hdr.symbol option) | G_RULE_LIST of unit ->  (Hdr.rule list list) | G_RULE of unit ->  (Hdr.rule list) | RHS_LIST of unit ->  ({ rhs:Hdr.symbol list,code:{ text:string,pos:Header.pos } ,prec:Hdr.symbol option }  list) | RECORD_LIST of unit ->  (string) | QUAL_ID of unit ->  (string) | MPC_DECLS of unit ->  (Hdr.declData) | MPC_DECL of unit ->  (Hdr.declData) | LABEL of unit ->  (string) | ID_LIST of unit ->  (Hdr.symbol list) | CONSTR_LIST of unit ->  ( ( Hdr.symbol * Hdr.ty option )  list) | BEGIN of unit ->  (string*Hdr.declData* ( Hdr.rule list list ) )
end
type svalue = MlyValue.svalue
type result = string*Hdr.declData* ( Hdr.rule list list ) 
end
structure EC=
struct
open LrTable
...
of  ( 0, ( ( _, ( MlyValue.G_RULE_LIST G_RULE_LIST1, _, G_RULE_LIST1right)) :: _ :: ( _, ( MlyValue.MPC_DECLS MPC_DECLS1, _, _)) :: ( _, ( MlyValue.HEADER HEADER1, HEADER1left, _)) :: rest671)) => let val  result = MlyValue.BEGIN (fn _ => let val  (HEADER as HEADER1) = HEADER1 ()
 ...
 in ((*#line 58.10 "yacc.grm"*)HEADER,MPC_DECLS,rev (map rev G_RULE_LIST)(*#line 388.1 "yacc.grm.sml"*)
)
end)
